      ...
      {orderData?.order?.length > 0 ? (
        <div>
          ...
          <div className="py-2 flex flex-col justify-center gap-2">
            {orderData?.order?.map((item: Products) => {
              ...
              return (
                <div
                  key={item.id}
                  className="py-2 border-b-[1px] border-gray-300 grid grid-cols-11 items-center"
                >
                  <div className="col-span-6 flex items-start gap-2 text-sm">
                    <Image
                      src={item.images?.[0] || "/allor-bbq.jpg"}
                      alt="product image"
                      width={500}
                      height={500}
          ...
          <Button
            ...
            className="mt-5 border-[1px] border-gray-500 py-1 px-4 font-medium hover:border-orange cursor-pointer duration-200"
          >
            Reset Order
          </Button>
        </div>
        ...
        <div className="py-10 bg-white text-black text-2xl text-center">
          <p>Nothing to show</p>
          <Link href={"/"}>
            <Button className="bg-black text-slate-100 w-44 h-12 rounded-full text-base font-semibold mt-2 hover:bg-orange duration-300">
              Continue Shopping
            </Button>
          </Link>
        </div>
