...
export const POST = async (request: NextRequest) => {
    ...
    try {
        ...
        const extractingItems = products.map((product: Products) => {
            ...
            const size = product.sizes.find(size => size.size === product.options.size);
            const basePrice = size ? size.price : 0;

            // Calculate the price of extra ingredients
            const extraIngredientsPrice = product.options.extraIngredients.reduce((acc: number, ingredient: string) => {
                const extraIngredient = product.extraIngredients.find(item => item.ingredient === ingredient);
                ...
                price_data: {
                    ...
                    product_data: {
                        ...
                        metadata: {
                            ...
                            extraIngredients: JSON.stringify(product.options.extraIngredients),
